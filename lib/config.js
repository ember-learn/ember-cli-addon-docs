...
module.exports = class AddonDocsConfig {
  ...
  // Used by deploy-staging code

  _getNormalizedRootUrl() {
    return this.getRootURL().replace(/^\/|\/$/g, '');
  }

  _currentDeployVersion() {
    let curpath = path.join('versions', this.getVersionPath());
    let name = this.getVersionName();
    let sha = this.repoInfo.sha;
    let tag = this.repoInfo.tag;
