...
const flattenParams = require('../models/utils/flatten-params');
  ...
  build() {
    ...
    for (let item of document.classitems) {
      if (item.class) {
        ...
        if (item.yields) {
          if (!(klass instanceof Component)) {
            project.ui.writeWarnLine('Attempted to add yields to a non-component');
          } else {
            klass.yields = flattenParams(item.yields);
          }
        } else if (Method.detect(item)) {
