...
function removeHiddenDocs(modules) {
  eachRelationship(modules, (relationship) => {
    _.remove(relationship, (c) =>
      c.tags.find((t) => t.name === 'hide' || t.name === 'hideDoc'),
    );

    eachRelationship(relationship, (subRelationship) => {
      _.remove(subRelationship, (c) =>
        c.tags.find((t) => t.name === 'hide' || t.name === 'hideDoc'),
      );
    });
  });
...
module.exports = class DocsCompiler extends CachingWriter {
  ...
  build() {
    ...
    let collectedDocs = generatedDocs.reduce(
      (collected, payload) => {
    ...
    fs.writeJsonSync(
      ...
      Serializer.serialize('project', project),
    );
  }
};
