...
function generateModuleNavigationItems(collection) {
...
function generateNavigationIndex(modules, klasses) {
  ...
  modules.forEach((m) => {
    ...
    let classItems = generateModuleNavigationItems(
      m.classes.filter(c => !c.file.match(/controllers\/|mixins\/|models\/|services\//))
    );

    let functionItems = generateModuleNavigationItems(m.functions);
    let variableItems = generateModuleNavigationItems(m.variables);
...
module.exports = class DocsCompiler extends CachingWriter {
  ...
  build() {
    let parentAddon = this.parentAddon;
