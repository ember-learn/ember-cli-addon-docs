...
module.exports = class AddonDocsDeployPlugin {
  ...
  _verifyRootFiles(stagingDirectory) {
    ...
    redirectContents = redirectContents.replace(/\bADDON_DOCS_OLD_PATHS\b/, JSON.stringify(oldPaths));
  ...
  _discoverOldPaths(stagingDirectory) {
    try {
      ...
      return Object.keys(versions).map(key => versions[key].oldpath).filter(Boolean);
    } catch (err) {
