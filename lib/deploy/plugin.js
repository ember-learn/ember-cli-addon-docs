...
module.exports = class AddonDocsDeployPlugin {
  ...
  willUpload(context) {
    ...
    return this._copyExistingFiles(context, relativeBuildDestination)
      .then(() => maybeMigrateSiteFormat(context))
  ...
  _verifyRootFiles(stagingDirectory) {
    ...
    let oldPaths = this._discoverOldPaths(stagingDirectory);

    redirectContents = redirectContents.replace(/\bADDON_DOCS_SEGMENT_COUNT\b/g, segmentCount);
    redirectContents = redirectContents.replace(/\bADDON_DOCS_OLD_PATH\b/, JSON.stringify(oldPaths));
  ...
  _discoverOldPaths(stagingDirectory) {
    try {
      ...
      return versions.map(v => v.oldPath).filter(Boolean);
    } catch (err) {
  ...
  _updateVersionsManifest(stagingDirectory) {
    ...
    /*let version = this._migrateVersion(); */
  ...
  _copyBuildOutput(context, stagingDirectory, relativeBuildDestination) {
    ...
    /*let deployVersion = this._migrateVersion(); */
