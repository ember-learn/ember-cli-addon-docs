...
function rewriteVersionsJSON(stagingDirectory){
  let versions = `${stagingDirectory}/versions.json`;
  Object.keys(versions).map(key => versions[key].path).set(path.join('versions', versions.path));
  // contents.forEach(key => {
  //   contents.set(key, versions.newPath);
  // })
