...
function rewriteVersionsJSON(stagingDirectory){
  ...
  let contents = Object.keys(versions).map(key => versions[key].path).filter(Boolean);
  contents.forEach(oldPath => {
    let newPath = path.join('versions', oldPath);
    versions[oldPath].path = newPath;
  })
